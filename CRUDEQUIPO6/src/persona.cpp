...
void persona::insert()
{
	...
	cin>>address;
	file.open("registroDeParticipantes.txt", ios::app | ios::out);
...
void persona::display()
{
	...
	file.open("registroDeParticipantes.txt",ios::in);
	if(!file)
	{
		cout<<"\n\t\t\tNo hay información...";
		file.close();
...
void persona::modify()
{
	...
	else
	{
		...
		file1.close();
		file.close();
		remove("registroDeParticipantes.txt");
		rename("registro.txt","registroDeParticipantes.txt");
	}
}
